...
const ScheduleCalendarSection = ({
    ...
    const [selectedDate, setSelectedDate] = useState(currentDate.getDate()); // 현재 날짜로 초기화
        ...
        <Section>
            <SectionHeader>
                <HeaderLeft>
                    ...
                    <Title>
                        {currentYear} {currentMonth}월 스케줄
                    </Title>
                </HeaderLeft>
                <HeaderRight>
                    <MonthNavigation>
                ...
                <CalendarGrid>
                    {scheduleData.map((dayData, index) => (
                        <CalendarDay
                            ...
                            $isToday={dayData.date && isToday(dayData.date)}
                            $isSunday={dayData.day === '일'}
                            $isSaturday={dayData.day === '토'}
                            $isOtherMonth={dayData.isOtherMonth}
                            onClick={() =>
                                dayData.date &&
                                handleDateClick(dayData.date, dayData)
                            }
                        >
                            {dayData.date && (
                                <DateNumber
                            ...
                            {dayData.employees.length > 0 && (
                                <EmployeeNames>
                                    {dayData.employees.map((name, empIndex) => (
                                        <EmployeeName key={empIndex}>
                                            {name}
                                        </EmployeeName>
                                    ))}
                                </EmployeeNames>
...
const CalendarDay = styled.div`
    ...
    &:hover {
        background: ${(props) => (props.$isOtherMonth ? '#f8f9fa' : '#f5f5f5')};
    }
`;
